...
func GenerateID(randomize bool) string  {
	suffix := "0"
	if randomize {
...
func PrepareCC(sdk *fabsdk.FabricSDK,user fabsdk.ContextOption,orgName string,chaincodeID string) error  {
	...
	if instantiated {
		err := resetCC(sdk, user, orgName, channelID, chaincodeID, resetArgs)
		if err != nil {
			return errors.WithMessage(err, "Resetting  chaincode failed")
		}
		return nil
	}

	fmt.Printf("Installing and instantiating  chaincode...")
	start := time.Now()
	...
	err = InstallCC(orgContexts, chaincodeID)
	if err != nil {
		return errors.WithMessage(err, "Installing  chaincode failed")
	}

	err = InstantiateCC(orgContexts, channelID, chaincodeID, ccPolicy)
	if err != nil {
		return errors.WithMessage(err, "Instantiating  chaincode failed")
	}
...
// InstallChaincode installs the  chaincode to all peers in the given orgs
func InstallCC(orgs []*OrgContext, ccID string) error {
...
// InstantiateChaincode instantiates the  CC on the given channel
func InstantiateCC(orgs []*OrgContext, channelID, ccID, ccPolicy string, collConfigs ...*cb.CollectionConfig) error {
...
func resetCC(sdk *fabsdk.FabricSDK, user fabsdk.ContextOption, orgName string, channelID string, chainCodeID string, args [][]byte) error {
